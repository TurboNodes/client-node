...
// notifyIconIdentifier identifies our icon to Shell_NotifyIconGetRect.
// https://learn.microsoft.com/en-us/windows/win32/api/shellapi/ns-shellapi-notifyiconidentifier
...
// iconRect asks the shell where it drew our icon. This can legitimately fail —
// the icon may be hidden in the notification-area overflow flyout, in which
// case there is no on-screen rectangle to report.
func iconRect() (x, y, w, h float64, ok bool) {
	...
	id := notifyIconIdentifier{
		Size: uint32(unsafe.Sizeof(notifyIconIdentifier{})),
		Wnd:  nid.Wnd,
		ID:   nid.ID,
	}
	var r struct{ Left, Top, Right, Bottom int32 }

	// S_OK is 0; anything else means the shell could not locate the icon.
	ret, _, _ := pShellNotifyIconGetRect.Call(
	...
	return float64(r.Left), float64(r.Top), float64(r.Right - r.Left), float64(r.Bottom - r.Top), true
}

// setVisible adds or removes the notification-area icon. Windows has no
// visibility flag for a tray icon, so hiding means deleting it and showing
// means adding it back; the window class and message loop stay untouched.
func setVisible(visible bool) {
	...
	if visible {
		if err := wt.nid.add(); err != nil {
			log.Printf("systray error: failed to show tray icon: %v\n", err)
		}
		return
	}
	if err := wt.nid.delete(); err != nil {
		log.Printf("systray error: failed to hide tray icon: %v\n", err)
	}
}
