...
- (NSWindow *)statusItemWindow {
  return self->statusItem.button.window;
}

- (void)setVisible:(NSNumber *)visible {
  self->statusItem.visible = [visible boolValue];
}
...
void getIconRect(double* x, double* y, double* w, double* h, int* valid) {
  void (^read)(void) = ^{
    ...
    NSWindow *win = [d statusItemWindow];
    if (win == nil) {
      *valid = 0;
      return;
    }
...
void setVisible(bool visible) {
  runInMainThread(@selector(setVisible:), (id)[NSNumber numberWithBool:visible]);
}
