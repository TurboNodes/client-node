...
// setVisible toggles the StatusNotifierItem between Active and Passive.
// Passive is the SNI way of saying "do not show me": the item stays exported
// and registered, so showing it again is just another property change rather
// than a re-registration with the watcher.
func setVisible(visible bool) {
	...
	// SetMust, not Set: Status is exported read-only to peers on the bus and
	// Set enforces that. SetMust is the same path the icon updates take.
	props.SetMust("org.kde.StatusNotifierItem", "Status", status)

	if conn == nil {
		return
	}
	err := notifier.Emit(conn, &notifier.StatusNotifierItem_NewStatusSignal{
		Path: path,
		Body: &notifier.StatusNotifierItem_NewStatusSignalBody{Status: status},
	})
	if err != nil {
		log.Printf("systray error: failed to emit new status signal: %s\n", err)
	}
}
